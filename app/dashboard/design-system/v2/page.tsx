...
export default function DesignSystemV2() {
	...
	const [localColors, setLocalColors] = useState<{ name: string; hex: string; rgb: string }[]>([]);
	const [localFonts, setLocalFonts] = useState<string[]>([]);
	const [newColorName, setNewColorName] = useState("");
	const [newColorHex, setNewColorHex] = useState("#3b82f6");
	const [newFont, setNewFont] = useState("");
	const [lastSavedId, setLastSavedId] = useState<Id<"design_systems"> | null>(null);
		...
		try {
			if (lastSavedId) {
				...
				const id = await saveSystem({
					...
					colors: merged.colors.length > 0 ? merged.colors : [{ name: "Neutral", hex: "#71717a", rgb: "rgb(113, 113, 122)" }],
					fonts: merged.fonts.length > 0 ? merged.fonts : ["Inter, sans-serif"],
					description: merged.description || "Design system",
					goodThings: merged.goodThings,
					badThings: merged.badThings,
	...
	const addManualColor = () => {
		...
		setLocalColors((prev) => [...prev, { name: newColorName.trim() || hex, hex, rgb: hexToRgb(hex) }]);
		setNewColorName("");
	...
	const analyzeImages = async () => {
		if (images.length === 0 || !selectedProjectId) {
			toast.error(images.length === 0 ? "Nahrajte obrázky!" : "Vyberte projekt!");
			return;
		}
		setIsAnalyzing(true);
		try {
			const result = await analyzeDesign({ imageUrls: images.map((img) => img.url) });
	...
	const handleGenerateDesign = async () => {
		...
		try {
			const result = await generateDesign({ imageUrls: images.map((img) => img.url) });
	...
	return (
		<div className="flex flex-col min-h-screen bg-background text-foreground selection:bg-primary/20">
			...
			<div className="relative h-64 lg:h-80 overflow-hidden bg-slate-950 flex flex-col justify-end p-8 lg:p-16 border-b border-white/5">
				...
				<div className="relative z-20 space-y-4 max-w-7xl mx-auto w-full">
					...
					>
						...
						<h1 className="text-4xl lg:text-6xl font-black uppercase tracking-tighter italic">Design System Lab</h1>
					</motion.div>
					<p className="text-muted-foreground max-w-xl text-lg font-medium opacity-60">
						Definujte vizuálnu DNA vášho projektu pomocou manuálnych tokenov alebo AI analýzy.
					</p>
				</div>
			</div>
			...
			<main className="flex-1 max-w-7xl mx-auto w-full p-8 lg:p-16 -mt-12 relative z-30">
				...
				<Card className="mb-12 p-6 bg-background/60 backdrop-blur-3xl border-border/50 rounded-3xl shadow-2xl flex flex-col md:flex-row items-center justify-between gap-8">
					<div className="flex flex-col gap-1.5 flex-1">
						<label className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40">Aktívny Projekt</label>
					...
					<div className="flex gap-3">
						<Button onClick={persistSystem} disabled={!selectedProjectId || isSaving} className="h-14 px-8 rounded-2xl font-black uppercase tracking-widest text-xs gap-2">
							{isSaving ? <RefreshCw className="size-4 animate-spin" /> : <Save className="size-4" />}
							Uložiť Systém
						</Button>
						<Button variant="outline" onClick={() => setIsNoteOpen(true)} className="h-14 w-14 rounded-2xl border-border/50">
					...
					<div className="inline-flex p-1.5 bg-muted/30 backdrop-blur-xl rounded-[2rem] border border-border/50">
						<button 
							...
							className={cn(
								...
								activeTab === "assets" ? "bg-background shadow-xl text-primary scale-105" : "text-muted-foreground hover:text-foreground"
							)}
						>
						...
						<button 
							...
							className={cn(
								"flex items-center gap-3 px-8 py-3 rounded-3xl transition-all duration-500 font-black uppercase tracking-[0.1em] text-xs",
								activeTab === "visual" ? "bg-background shadow-xl text-primary scale-105" : "text-muted-foreground hover:text-foreground"
							)}
						>
					...
					{activeTab === "assets" ? (
						...
						>
							...
							<div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
								...
								<Card className="p-8 rounded-[2.5rem] bg-background/40 border-border/50 space-y-8">
									<div className="flex items-center justify-between">
										<h2 className="text-xl font-black uppercase tracking-tight flex items-center gap-3">
											<div className="size-2 rounded-full bg-blue-500 animate-pulse" /> Paleta Farieb
										</h2>
										<Plus className="size-5 opacity-20" />
									</div>
									<div className="space-y-6">
										<div className="flex gap-3">
											<Input placeholder="Názov farby" value={newColorName} onChange={(e) => setNewColorName(e.target.value)} className="h-12 rounded-xl bg-muted/20 border-none" />
											<Input type="color" value={newColorHex} onChange={(e) => setNewColorHex(e.target.value)} className="w-16 h-12 p-1 rounded-xl bg-muted/20 border-none cursor-pointer" />
											<Button onClick={addManualColor} className="h-12 rounded-xl px-6">Pridať</Button>
										</div>
										<div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
											{merged.colors.map((c, i) => (
												<div key={i} className="group relative p-3 rounded-2xl bg-muted/10 border border-border/5 hover:border-primary/20 transition-all">
													<div className="h-16 w-full rounded-xl mb-3 shadow-inner" style={{ backgroundColor: c.hex }} />
													<p className="text-[10px] font-black uppercase truncate">{c.name}</p>
													<p className="text-[9px] font-mono opacity-40 uppercase tracking-tighter">{c.hex}</p>
													<button 
														onClick={() => setLocalColors(prev => prev.filter((_, idx) => idx !== i - (system?.colors.length || 0)))}
								...
								<Card className="p-8 rounded-[2.5rem] bg-background/40 border-border/50 space-y-8">
									<h2 className="text-xl font-black uppercase tracking-tight flex items-center gap-3">
										<div className="size-2 rounded-full bg-purple-500 animate-pulse" /> Typografia
									</h2>
									<div className="space-y-6">
										<div className="flex gap-3">
											<Input placeholder="Názov fontu (napr. Inter)" value={newFont} onChange={(e) => setNewFont(e.target.value)} className="h-12 rounded-xl bg-muted/20 border-none" />
											<Button onClick={addManualFont} variant="secondary" className="h-12 rounded-xl px-6">Pridať</Button>
										</div>
										<div className="space-y-3">
											{merged.fonts.map((f, i) => (
												<div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-muted/10 border border-border/5 group">
													<span className="font-medium text-lg" style={{ fontFamily: f }}>{f}</span>
													<button className="opacity-0 group-hover:opacity-100 text-red-500"><Trash2 className="size-4" /></button>
												</div>
											))}
										</div>
									</div>
							...
							<div className="space-y-6">
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
									{historyList?.map((h) => (
										<Card key={h._id} className="p-6 rounded-[2rem] bg-background/40 border-border/50 hover:border-primary/30 transition-all cursor-pointer group relative overflow-hidden">
											<div className="absolute top-0 right-0 p-4 opacity-0 group-hover:opacity-100 transition-all">
												<Trash2 className="size-4 text-red-500 hover:scale-110" />
											</div>
											<div className="space-y-3">
												<p className="font-black uppercase tracking-tight text-sm">{h.projectName || "Bez názvu"}</p>
												<p className="text-[10px] text-muted-foreground opacity-60 font-medium">{new Date(h._creationTime).toLocaleDateString()}</p>
												<div className="flex gap-1.5 mt-4">
													{h.colors.slice(0, 5).map((c, idx) => (
														<div key={idx} className="size-6 rounded-full border border-white/10" style={{ backgroundColor: c.hex }} />
													))}
												</div>
											</div>
										</Card>
							...
							<div className="space-y-8">
								...
								>
									...
									{images.length === 0 ? (
										<div className="space-y-8">
											...
											<div className="space-y-3">
												<h3 className="text-2xl font-black uppercase tracking-tight">Presuňte inšpiráciu sem</h3>
												<p className="text-muted-foreground font-medium opacity-60">Naša AI zanalyzuje štýl, farby a atmosféru z vašich obrázkov.</p>
											</div>
										</div>
									) : (
										<div className="flex flex-wrap justify-center gap-6">
											{images.map((img) => (
												<div key={img.id} className="relative group/img size-48 rounded-[2rem] overflow-hidden shadow-2xl border border-white/10">
													<img src={img.url} className="size-full object-cover transition-all group-hover/img:scale-110" alt="Insp" />
													<button 
								...
								{images.length > 0 && (
									<div className="flex flex-col md:flex-row items-center justify-center gap-6">
										...
										>
											{isAnalyzing ? <RefreshCw className="size-5 animate-spin" /> : <Wand2 className="size-6 group-hover:rotate-12 transition-transform" />}
											Analyzovať Štýl
										</Button>
										<Button 
											size="lg" 
										...
										>
											{isGeneratingDesign ? <RefreshCw className="size-5 animate-spin" /> : <Sparkles className="size-6 group-hover:scale-110 transition-transform" />}
											Generovať Design
										</Button>
									</div>
								)}
								...
								{system && (
									...
									>
										<Card className="p-8 rounded-[2.5rem] bg-primary/5 border-primary/20 space-y-4">
											<h4 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40">AI Analýza</h4>
											<p className="text-lg font-bold leading-relaxed">{system.description}</p>
										</Card>
										<Card className="p-8 rounded-[2.5rem] bg-green-500/5 border-green-500/20 space-y-4">
											<h4 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40 text-green-500">Silné Stránky</h4>
											<ul className="space-y-2">
												{system.goodThings?.slice(0, 3).map((t, i) => (
													<li key={i} className="text-xs font-bold flex items-center gap-2"><CheckCircle2 className="size-3 text-green-500" /> {t}</li>
												))}
											</ul>
										</Card>
										<Card className="p-8 rounded-[2.5rem] bg-amber-500/5 border-amber-500/20 space-y-4">
											<h4 className="text-[10px] font-black uppercase tracking-[0.3em] opacity-40 text-amber-500">Odporúčania</h4>
											<ul className="space-y-2">
												{system.suggestions?.slice(0, 3).map((t, i) => (
													<li key={i} className="text-xs font-bold flex items-center gap-2"><Lightbulb className="size-3 text-amber-500" /> {t}</li>
												))}
											</ul>
										</Card>
									</motion.div>
			...
			<QuickNoteDialog isOpen={isNoteOpen} onOpenChange={setIsNoteOpen} projectId={selectedProjectId as Id<"projects"> || undefined} />
		</div>
	);
}
